...
public class TestKeyboard {
    ...
    public String simpleTestTextWTab = "wwwwww\n\twwwww\nwwwwww";
    ...
    private void tabbing() {
        //shift tab
        doDeleteThingWithSelectCheckSelect(simpleTestTextWTab, true,
                "wwwwww\nwwwww\nwwwwww", 10, 10, null, null, null);

//        select tab
        doDeleteThingWithSelectCheckSelect(simpleTestText, false,
                "\twwwwww\nwwwwww\nwwwwww", 0, 1, "\tw", "w", "\tw");

        //select shift tab
        doDeleteThingWithSelectCheckSelect(simpleTestTextWTab, true,
                "wwwwww\nwwwww\nwwwwww", 10, 12, "ww", "ww", "ww");

        //select tab 3 lines
        doDeleteThingWithSelectCheckSelect(simpleTestText, false,
                "\twwwwww\n\twwwwww\n\twwwwww", 0, 20, "\twwwwww\n\twwwwww\n\twwwwww", simpleTestText, "\twwwwww\n\twwwwww\n\twwwwww");

        //shift tab 3 lines
        doDeleteThingWithSelectCheckSelect(simpleTestTextWTab, true,
                "wwwwww\nwwwww\nwwwwww", 0, 20, "wwwwww\nwwwww\nwwwwww", simpleTestTextWTab, "wwwwww\nwwwww\nwwwwww");

        //shift line with only middle one working
        doDeleteThingWithSelectCheckSelect("\twwwwww\n\twwwwww\n\twwwwww", true,
                simpleTestText, 0, 20, simpleTestText, "\twwwwww\n\twwwwww\n\twwwwww", simpleTestText);

    }

    private void doDeleteThingWithSelectCheckSelect(String setText, boolean shiftDown, String textAfter, int lineOneOffset, int lineTwoOffset, String selectTextAfter, String selectTextUndo, String selectTextRedo) {
        adapter.undoer = new Undoer();
        GUIHandler.setEditorText(setText);
        GUIHandler.setSelectedOffset(lineOneOffset, lineTwoOffset);
        pressButton((char) KeyEvent.VK_TAB, false, shiftDown);
        Assert.assertEquals(textAfter, GUIHandler.getEditorText());
        Assert.assertEquals(selectTextAfter, GUIHandler.getSelectedText());

        pressButton((char) KeyEvent.VK_Z, true, false);
        Assert.assertEquals(setText, GUIHandler.getEditorText());
        Assert.assertEquals(selectTextUndo, GUIHandler.getSelectedText());

        pressButton((char) KeyEvent.VK_Y, true, false);
        Assert.assertEquals(textAfter, GUIHandler.getEditorText());
        Assert.assertEquals(selectTextRedo, GUIHandler.getSelectedText());
    }
    ...
    private void backSpace() {
        ...
        //remove no char
        doDeleteThing(0, simpleTestText, KeyEvent.VK_BACK_SPACE, false, false, simpleTestText, 0, 0, 0);
        //remove the last char
        doDeleteThing(simpleTestText.length(), simpleTestText, KeyEvent.VK_BACK_SPACE, false, false, simpleTestText.substring(0, simpleTestText.length() - 1), simpleTestText.length() - 1, simpleTestText.length(), simpleTestText.length() - 1);
        //delete first word
        doDeleteThing(6, simpleTestText, KeyEvent.VK_BACK_SPACE, true, false, "\nwwwwww\nwwwwww", 0, 6, 0);
        //delete last word
        doDeleteThing(20, simpleTestText, KeyEvent.VK_BACK_SPACE, true, false, "wwwwww\nwwwwww\n", 14, 20, 14);
        //delete no word
        doDeleteThing(0, simpleTestText, KeyEvent.VK_BACK_SPACE, true, false, simpleTestText, 0, 0, 0);

        //selected
        //remove first char
        doDeleteThingWithSelect(simpleTestText, KeyEvent.VK_BACK_SPACE, false, false, simpleTestText.substring(2), 0, 2, 0, 0, 2);
        //remove last char
        doDeleteThingWithSelect(simpleTestText, KeyEvent.VK_BACK_SPACE, false, false, simpleTestText.substring(0, simpleTestText.length() - 2), simpleTestText.length() - 2, simpleTestText.length(), simpleTestText.length() - 2, simpleTestText.length() - 2, simpleTestText.length());
        //delete word
        doDeleteThingWithSelect(simpleTestText, KeyEvent.VK_BACK_SPACE, true, false, "www\nwwwwww\nwwwwww", 3, 6, 3, 3, 6);
        //delete last word
        doDeleteThingWithSelect(simpleTestText, KeyEvent.VK_BACK_SPACE, true, false, "wwwwww\nwwwwww\nwww", 14, 17, 14, 14, 17);
    ...
    private void delete() {
        ...
        //remove last char
        doDeleteThing(simpleTestText.length() - 1, simpleTestText, KeyEvent.VK_DELETE, false, false, simpleTestText.substring(0, simpleTestText.length() - 1), simpleTestText.length() - 1, simpleTestText.length(), simpleTestText.length() - 1);
        //delete first line
        ...
        //selected
        //remove first char
        doDeleteThingWithSelect(simpleTestText, KeyEvent.VK_DELETE, false, false, simpleTestText.substring(2), 0, 2, 0, 0, 2);
        //remove last char
        doDeleteThingWithSelect(simpleTestText, KeyEvent.VK_DELETE, false, false, simpleTestText.substring(0, simpleTestText.length() - 2), simpleTestText.length() - 2, simpleTestText.length(), simpleTestText.length() - 2, simpleTestText.length() - 2, simpleTestText.length());
        //delete first 2 lines
        doDeleteThingWithSelect(simpleTestText, KeyEvent.VK_DELETE, false, true, "wwwwww", 0, 14, 0, 0, 10);
        //delete last 2 lines
        doDeleteThingWithSelect(simpleTestText, KeyEvent.VK_DELETE, false, true, "wwwwww", 6, 20, 6, 10, 18);
        //delete line but holding ctrl
        //delete first line
        doDeleteThingWithSelect(simpleTestText, KeyEvent.VK_DELETE, true, true, "wwwwww", 0, 14, 0, 0, 10);
        //delete last line
        doDeleteThingWithSelect(simpleTestText, KeyEvent.VK_DELETE, true, true, "wwwwww", 6, 20, 6, 10, 18);
        //delete word
        doDeleteThingWithSelect(simpleTestText, KeyEvent.VK_DELETE, true, false, "www\nwwwwww\nwwwwww", 0, 3, 0, 0, 3);
        //delete last word
        doDeleteThingWithSelect(simpleTestText, KeyEvent.VK_DELETE, true, false, "wwwwww\nwwwwww\nwww", 17, 20, 17, 17, 20);
    }

    private void doDeleteThingWithSelect(String setText, int keyChar, boolean ctrlDown, boolean shiftDown, String textAfter, int positionAfter, int posAfterUndo, int posAfterRedo, int lineOneOffset, int lineTwoOffset) {
        GUIHandler.setEditorText(setText);
        GUIHandler.setSelectedOffset(lineOneOffset, lineTwoOffset);
        doDeleteThing(-1, setText, keyChar, ctrlDown, shiftDown, textAfter, positionAfter, posAfterUndo, posAfterRedo);
    }
